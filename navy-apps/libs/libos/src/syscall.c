...
intptr_t _syscall_(intptr_t type, intptr_t a0, intptr_t a1, intptr_t a2)
{
  ...
  asm volatile(SYSCALL : "=r"(ret) : "r"(_gpr1), "r"(_gpr2), "r"(_gpr3), "r"(_gpr4)); // 系统调用号存放在 a7 寄存器中 a0", "a1", "a2"：这些寄存器用于传递系统调用的参数
  return ret;
}
...
extern char _end; // _end符号通常由链接器定义，指向程序结束的地方，也是堆的起始地址
static uintptr_t program_break = (uintptr_t)&_end;
void *_sbrk(intptr_t increment)
{
  uintptr_t old_brk = program_break;
  uintptr_t new_brk = old_brk + increment;
  if (_syscall_(SYS_brk, new_brk, 0, 0) == 0) // 系统调用是SYS_brk, 它接收一个参数addr, 用于指示新的program break的位置
  {
