...
/*
尽管 handle_key 只处理一个按键并返回一个字符，但 keypress 函数（尤其在 "cook" 模式下）会累积这些字符，直到用户按下回车键。
keypress 会将每个字符逐一添加到一个缓冲区 input 中，直到检测到换行符 (\n) 为止
最终，keypress 返回的 res 是一个指向完整命令字符串的指针，而不是单个字符。
这就是 sh_handle_cmd 能够接收到完整字符串如 "echo hello" 的原因。
*/
